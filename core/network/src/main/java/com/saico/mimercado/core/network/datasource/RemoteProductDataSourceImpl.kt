...
class RemoteProductDataSourceImpl @Inject constructor(
    ...
    override suspend fun getProducts(query: String, page: Int): Result<List<Product>> = coroutineScope {
        try {
            ...
            val products = usdaResponse.foods.map { dto ->
                async {
                    ...
                    if (upc.isNotBlank()) {
                        try {
                            ...
                            if (offResponse.status == 1) {
                                imageUrl = offResponse.product?.imageFrontUrl ?: offResponse.product?.imageUrl ?: ""
                            }
                        } catch (e: Exception) {}
                    }
                    ...
                    Product(
                        ...
                        categoria = dto.foodCategory ?: "General",
                        imageUrl = imageUrl,
